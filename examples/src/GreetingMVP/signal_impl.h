...
public:
    ...
    SlotBase* connect(void(*func)());         // Сигнал владеет обёрткой
    SlotBase* connect(SlotBase* slot);        // Пользователь управляет памятью
    SlotBase* connect(SlotBase& slot);        // Пользователь управляет памятью (по ссылке)
